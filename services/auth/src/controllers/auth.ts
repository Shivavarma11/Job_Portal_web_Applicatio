...
export const registerUser = TryCatch(async (req, res, next) => {
    ...
    if (role === "recruiter") {
        ...
        VALUES (${name},${email},${hashPassword},${phoneNumber},${role}) RETURNING
        ...
        if (!file) {
            throw new ErrorHandler(400, "Resume is needed for jobseekers")
        }
        ...
        VALUES (${name},${email},${hashPassword},${phoneNumber},${role},${bio},${data.url},${data.resume_public_id}) RETURNING
...
export const loginUser = TryCatch(async(req,res,next)=>{
    ...
    u.profile_pic, u.subscription, ARRAY_AGG(s.name) FILTER (WHERE s.name IS NOT NULL) as skills FROM users
    u LEFT JOIN user_skills us ON u.user_id = us.user_id LEFT JOIN skills s ON us.skill_id = s.skill_id
...
export const forgotPassword = TryCatch(async(req,res,next)=>{
    ...
    res.json({
        message : "If mail exists, we have sent a reset link"
    })
})
...
export const resetPassword = TryCatch(async(req,res,next)=>{
    ...
    try {
        ...
        throw new ErrorHandler(400,"Token expired");
    }
